...
function Activity() {
  ...
  const columns = [
    {
      ...
      dataIndex: "active_on",
      render: (_, { active_on }) => {
        return active_on ? moment(active_on).format("DD-MM-YYYY hh:mm A") : "-";
      },
    },
    {
      title: "Inactive On",
  ...
  //       Name:
  //         row?.user_id?.name?.split(" ")[0] && row?.user_id?.name?.split(" ")[1]
  //           ? row?.user_id?.name?.split(" ")[0] +
  //             " " +
  //             row?.user_id?.name?.split(" ")[1]
  //           : "-",
  //       "Device Type": row?.device_type ? row?.device_type : "-",
  //       "Active on": row?.active_on
  //         ? moment(row?.active_on).format("DD-MM-YYYY HH:mm A")
  //         : "-",
  //       "InActive On": row?.inactive_on
  //         ? moment(row?.inactive_on).format("DD-MM-YYYY HH:mm A")
  //         : "-",
  //     }));
