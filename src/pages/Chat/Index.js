      ...
      <div className="tabled h-100 chatMainwraper">
        <Row gutter={[24, 0]} className="h-100">
          <Col xs={24} className="h-100">
            ...
            >
              <div className="chatMainOuter d-flex" style={{justifyContent:"start"}}>
                        ...
                        users.map(user =>
                          ...
                          >
                            <div className="imgOuter">
                              <img src={user.image ? apiPath.assetURL + user.image : "https://writestylesonline.com/wp-content/uploads/2018/11/Three-Statistics-That-Will-Make-You-Rethink-Your-Professional-Profile-Picture-1024x1024.jpg"} />
                            </div>
  ...
  return (
    <>
      <div className="contactOuter h-100 d-flex flex-column flex-nowrap">
        ...
        <div className="contactBody h-100 flex-fill">
          {
            loading ? [1, 2, 3, 4].map(item => <Skeleton key={item} active />)
              :
              users?.length ?
                users.map(user =>{
...
const ChatDetail = ({ selected, selectedInbox, user }) => {
  ...
  useEffect(() => {
    ...
    setLoadind(true)
    setIsActive(true)
    const subCollectionRef = collection(chatListRef, `${selectedInbox}_${selected}`);
  ...
  return (
    <div className={`chatingOuter ${isActive ? `active` : ''} h-100 d-flex flex-column flex-nowrap`}>
      <div className="chatingHeader">
        <button onClick={() => setIsActive(false)} type="button" class="backArrowBtn">
          ...
          <div className="name">{user?.name}</div>
          {
            isOnline ?
              (isOnline.is_online ?
                ...
                <div className="status online">Last seen {formatDate(isOnline?.last_seen?.toDate())}</div>)
              :
              <div className="status online">Online</div>
          }
        </div>
        ...
        data.type == "text" ?
          <div className="msgItem">
            ...
            <div className="msgTime">
              {formatDate(data?.created_at?.toDate())}</div>
          </div>
          :
    ...
    <>
      {
        data.type = 'text' ?
          <div className="msgItem sent">
            <div className="txt">{data.message}</div>
            <div className="msgTime">{formatDate(data?.created_at?.toDate())}</div>
          </div>
          :
